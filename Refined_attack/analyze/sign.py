...
results_path = r'./results_sign_{0}_{1}_{2}_{3}/key_{4}/'.format(n, min_samples, max_samples, step_samples, n_files)
...
for ii in range(n_files, n_files+1):
    ...
    for iteration in [max_samples]:
        ...
        for i in range(min_samples):
            ...
            for row in rows:
                index = indices[row]
                # filtered by sign leakage and then transform the signatures
                if z_read[index] > 0:
        ...
        for cur_col in range(total_cols):
            ...
            dotnumber = zeros(count, dtype=float64)
            signature_valid_new = zeros((count, 2 * n), dtype=int64)

            j = 0
            for i in range(count):
                dotnumber[i] = dot(signature_valid[i], egi_vector_T_b0)
                if abs(dotnumber[i]) < norm_th:
                    signature_valid_new[j] = signature_valid[i]
                    j = j + 1
            signature_valid_new_lim = zeros((j, 2 * n), dtype=int64)
            for i in range(j):
                signature_valid_new_lim[i] = signature_valid_new[i]
            # print("j = ", j)

            cov_result = cov(signature_valid_new_lim, rowvar=False)
            egi_result, egi_vector = linalg.eig(cov_result)
